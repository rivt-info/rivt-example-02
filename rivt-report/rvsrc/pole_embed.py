...
def Depth_nonconstrained(d, P, h, dia, PFP, tol):
    ...
    S1 = PFP * d / 3
    A = 2.34 * P / (S1 * b)
    dnew = 0.5 * A * (1 + (1 + (4.36 * h / A)) ** 0.5)

    # added d as a factor for consistent units
    while abs(d - dnew) > 0.0001 * d:
        d = dnew
        S1 = PFP * d / 3
        A = 2.34 * P / (S1 * b)
        dnew = 0.5 * A * (1 + (1 + (4.36 * h / A)) ** 0.5)
...
def Depth_constrained(d, P, h, dia, PFP):
